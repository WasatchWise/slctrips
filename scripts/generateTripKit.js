...
async function generateTripKitToNotion(config) {
  try {
...
Make the content feel like a premium travel guide that users would pay for.`
        ...
        {
          role: "user",
          content: `Create a TripKit for theme: "${config.theme}" using this destination data: ${JSON.stringify(destinations.slice(0, 5))}`
        }
      ]
    });
